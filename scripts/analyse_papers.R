...
paper_repo <- papers %>%
  ...
  mutate(readme = repair_encoding(readme_old, from = "UTF-8")) %>%
...
gl_comformity_readme <- paper_repo %>%
  ...
  select(-date, -authors, -Timestamp, -repo_url, -readme_old, -private, -readme_html)
...
# Idea: vector of vector - elements of inner vectors are being connected with AND. Outer vectors with OR.
...
repos %>%
  right_join(papers, by = "paper_url") %>%
  filter(str_detect(readme, "NeurIPS")) %>%
  View()
